...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // If Caspio honors the browser's Range request, pass its 206 response straight through.
    ...
    // Some Caspio file responses ignore Range and return the entire file with 200.
    // Normalize that into a standards-compliant 206 response so HTML5 video can play/seek.
    ...
    return new Response(sliceReadableStream(caspioResponse.body, start, end), {
      status: 206,
      headers
    });
  } catch (error) {
