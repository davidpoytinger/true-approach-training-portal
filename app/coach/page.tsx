...
async function publishSession(formData: FormData) {
  ...
  const videoNote = String(formData.get("videoNote") ?? "").trim();
  ...
  try {
    ...
    if (video instanceof File && video.size > 0) {
      ...
      uploadForm.append("Files", video, video.name);
      ...
      if (!uploadedFile?.fullFilePath) throw new Error("Caspio did not return a file path");

      await caspioFetch(`/tables/${SESSION_VIDEOS_TABLE_ID}/records`, {
        method: "POST",
        body: JSON.stringify({ SessionID: sessionId, VideoFile: uploadedFile.fullFilePath, Title: videoTitle || video.name, CoachNote: videoNote || null, DisplayOrder: 1 })
      });
    }
  } catch (error) {
