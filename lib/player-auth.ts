...
export type Access = {
  ...
  IsActive?: boolean | number | null;
  CanEditProfile?: boolean | number | null;
...
export function verifyPassword(p: string, stored: string) { try { const [salt, hash] = stored.split(":"); return timingSafeEqual(scryptSync(p, salt, 64), Buffer.from(hash, "hex")); } catch { return false; } }

export async function setAccountSession(id: number) {
...
export async function getAccountId() {
  ...
  if (!sig || sig.length !== expected.length || !timingSafeEqual(Buffer.from(sig), Buffer.from(expected)) || Number(expires) < Math.floor(Date.now() / 1000)) return null;
  const n = Number(id);
...
export async function getPlayerAccess(id: number) {
  const r = await caspioFetch<Response<Access>>(`/tables/${PLAYER_ACCESS_TABLE_ID}/records?select=AccessID,PlayerID,AccountID,Relationship,AccessLevel,IsPrimary,IsActive,CanEditProfile,CanAddSessions,CanEditPlayerAddedSessions,CanManageAccess&where=AccountID=${id}&limit=100`);
  return (r.data ?? []).filter(x => x.PlayerID > 0 && x.IsActive !== false && x.IsActive !== 0);
}
